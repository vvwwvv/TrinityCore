...
class HeartOfFlameCheck : public std::unary_function<WorldObject*, bool>
{
public:
...
//		if (!quest || player->GetQuestStatus(quest->GetQuestId) != QUEST_STATUS_INCOMPLETE)
//			return true;
...
public:
	...
	struct npc_ragnaros_dreadflameAI : public PassiveAI
	{
		...
		std::vector<DynamicObject*> GetDreadflamesToExtinguish(float radius) const
		{
			std::vector<DynamicObject*> dreadflames;
			std::vector<DynamicObject*> dreadflamesVector = me->GetDynObjects(SPELL_DREADFLAME_SPAWN);
			for (auto dreadflame : dreadflamesVector)
			{
				std::list<Player*> players;
				Trinity::AnyPlayerInObjectRangeCheck checker(dreadflame, radius);
				Trinity::PlayerListSearcher<Trinity::AnyPlayerInObjectRangeCheck> searcher(dreadflame, players, checker);
				dreadflame->VisitNearbyWorldObject(radius, searcher);
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_PHASE_FOUR_EVENT_1:
					...
					me->PlayOneShotAnimKit(ANIM_KIT_EMERGE_HC);
...
public:
	...
	struct npc_ragnaros_son_of_flameAI : public PassiveAI
	{
		...
		void SpellHit(Unit* caster, const SpellInfo* spell) override
		{
			...
			me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
			me->PlayOneShotAnimKit(ANIM_KIT_UNK);
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = _events.ExecuteEvent())
			{
				...
				case EVENT_HEROIC_MALFURION_1:
					...
					me->PlayOneShotAnimKit(ANIM_KIT_TALK);
...
public:
	...
	class spell_dreadflame_deluge_control_aura_SpellScript : public SpellScript
	{
		...
		void HandleDeluge()
		{
			...
			for (auto dreadflame : ai->GetDreadflamesToExtinguish(3.0f))
			{
		...
		std::set<ObjectGuid> dreadflamesRemoveList;
	};
...
public:
	...
	class spell_breadth_of_frost_player_aura_SpellScript : public SpellScript
	{
		...
		void OnSpellHit(SpellEffIndex effIndex)
		{
			GetHitUnit()->RemoveAurasDueToSpell(GetSpellInfo()->Effects[EFFECT_1].BasePoints);
		}
...
public:
	...
	class spell_fl_ragnaros_molten_inferno_SpellScript : public SpellScript
	{
		...
		void ChangeDamage()
		{
			float maxRange = GetSpellInfo()->Effects[EFFECT_0].CalcRadius();
